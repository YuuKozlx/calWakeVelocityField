...
for kp = 5:5:100
    ...
    for omega = 0.000005:0.00001:1.6
        ...
        for index = 1:length(hp)
            ...
            % 注意：这种针对A的计算方法，当omega == Np时，由于跳过了所有判断标志的处理操作,即不进行赋值操作,理应有a11 = a12 = a21 = a22 = 0,如此在计算
            % F = a0*a1*a2...an-1*an时会有一个矩阵ai等于0矩阵，但是原论文中并没有对omega == Np的情况进行讨论.
            % 为避免在计算中出现这种问题，可考虑在并行优化时给omega增加一个小于步长的偏置量，并在避免nv频率的取值与omega的各个取值重合。
            % 而此程序中在遇到 omega == Np的位置时作为替代，a11 a12 a21 a22 取值为前一个Omega 对应取值
